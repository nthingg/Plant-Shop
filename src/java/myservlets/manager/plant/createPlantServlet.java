        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (cateID != null && !cateID.equals("") && !cateID.isEmpty()) {
                ...
                if (checkCate == false) {
                    ...
                    request.getRequestDispatcher("plantsCreate.jsp").forward(request, response);
                }
            }

            boolean result = false;
            try {
                result = PlantDao.insertPlant(plantName, plantPrice, plantImg, plantDescription, plantStatus, cateID);
            } catch (Exception ex) {
                ex.printStackTrace();
            }
